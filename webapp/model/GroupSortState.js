sap.ui.define([
	...
	return BaseObject.extend("sapui5.SAPUI5_FINAL.model.GroupSortState", {
		 ...
		 * @class
		 * @public
		 * @alias sapui5.SAPUI5_FINAL.model.GroupSortState
		 */
		...
		/**
		 * Sorts by Titel, or by ProjectId
		 *
		...
		/**
		 * Groups by ProjectId, or resets the grouping for the key "None"
		 *
		 * @param sKey - the key of the field used for grouping
		 * @returns {sap.ui.model.Sorter[]} an array of sorters
		 */
		group: function (sKey) {
			...
			if (sKey === "ProjectId") {
				...
				this._oViewModel.setProperty("/sortBy", "ProjectId");

				aSorters.push(
					new Sorter("ProjectId", false,
						this._fnGroupFunction.bind(this))
				);
			} else if (sKey === "None") {
				// select the default sorting again
				this._oViewModel.setProperty("/sortBy", "Titel");
			}
